...
"""Regenerate everything downstream of roadmap.json + the README changelog.
  ...
  roadmap.json  ─┐
  README.md      ├─►  assets/roadmap.svg   (the cool roadmap image)
  (## Changelog) ─┘    site/data.json       (what the website renders dynamically)
...
STATUS = {
    "done":        {"label": "✓ done",        "fill": "#e9f7ef", "stroke": "#16a34a", "pill": "#16a34a"},
    "in_progress": {"label": "▸ in progress", "fill": "#eef4ff", "stroke": "#3b6fd4", "pill": "#3b6fd4"},
    "planned":     {"label": "○ planned",     "fill": "#f6f6f5", "stroke": "#c4c4bf", "pill": "#9a9a96"},
}
...
def render_svg(data: dict) -> str:
    ...
    for i, ph in enumerate(phases):
        ...
        P.append(f'<text x="{x+16}" y="{top+30}" font-size="12" font-weight="800" '
                 f'letter-spacing=".08em" fill="{st["pill"]}">{ph["id"]}</text>')
